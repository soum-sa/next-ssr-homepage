...
export const MobileBanners = ({ banners }: { banners: HomepageDataResponse['banners'] }) => {
  ...
  if (usedBanners?.length === 1) {
    return (
      <div className={mobileSingleImageContainerStyle}>
        <MobileUpperBannerImage banner={usedBanners[0]} type="single" index={0} />
      </div>
    );
  }
  ...
  return (
    <div className={mobileCarouselStyles}>
      <div className={emblaStyles} ref={emblaRef}>
        <div className={emblaContainerStyles} style={{ direction: isRTL ? 'rtl' : 'ltr' }}>
          {usedBanners?.map((banner, index) => (
            <div className={mobileSlideStyles} key={banner.id}>
              <MobileUpperBannerImage banner={banner} index={index} type="multiple" />
            </div>
          ))}
        </div>
      </div>
      <div className={mobileDotsStyles}>
        {scrollSnaps.map((_, index) => (
          <CarouselDotButton
            ...
            className={cx(mobileDotStyles, index === selectedIndex ? mobileDotSelectedStyles : '')}
